    ...
    def forward(self, x, coords=None, pos_embs=None):
        for layer in self.layers:
            if self.config.mamba_2d:
                x = layer(x, coords, self.pad_token.to(x.dtype), pos_embs)
            else:
                x = layer(x, coords)
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        if self.config.use_cuda:
            try:
                if self.config.mamba_2d:
                    from .pscan_2d import selective_scan_fn as selective_scan_fn
    ...
    def forward(self, x):
        ...
        if self.config.use_cuda:
            output = self.out_proj(y.to(self.out_proj.weight.dtype)) # (B, L, D)
            return output # the rest of the operations are done in the ssm function (fused with the CUDA pscan)

        # z branch
        z = F.silu(z)   # BS, W, H, ED
        output = y * z
        output = self.out_proj(output.to(self.out_proj.weight.dtype)) # (B, L, D)
...
def reconstruct_2d_wsi(feats, coords, max_w, max_h, patch_size, pad_token):
    ...
    """
    # 计算实际的max_w和max_h，基于coords中的最大值
    actual_max_w = int(coords[:, 0].max().item()) + patch_size
    actual_max_h = int(coords[:, 1].max().item()) + patch_size
