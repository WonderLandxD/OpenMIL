...
def create_model(pretrained: str, model_arch: str, in_chans: int, local_dir: str = os.path.join(os.path.expanduser("~"), ".cache/"), **kwargs):
    ...
    has_pretrained = os.path.exists(local_path)
    model = timm.create_model(
        model_arch,
        pretrained=False,
        in_chans=in_chans,
        init_vit_weights=not has_pretrained,
        **kwargs,
    )

    if has_pretrained:
        state_dict = torch.load(local_path, map_location="cpu", weights_only=False)["model"]
