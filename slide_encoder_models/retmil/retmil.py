...
class RetMIL(nn.Module):
    ...
    """
    def __init__(self, dim_in, dim_hidden, num_heads, chunk_size=512, num_classes=2):
        super().__init__()
        self.dim_in = dim_in
        ...
        self.local_retention = Retention(embed_dim=self.dim_hidden, num_heads=self.num_heads, gate_fn='swish')
        self.local_attn_pool = Attn_Net_Gated(L=self.dim_hidden, D=self.dim_hidden // 2, dropout=0.25, n_classes=1)
        self.global_retention = Retention(embed_dim=self.dim_hidden, num_heads=self.num_heads, gate_fn='swish')
        self.global_attn_pool = Attn_Net_Gated(L=self.dim_hidden, D=self.dim_hidden // 2, dropout=0.25, n_classes=1)
        self.classifier = nn.Linear(self.dim_hidden, self.num_classes, bias=False)

    def forward(self, input_dict):
        x = self._fc1(input_dict['feats'])
...
if __name__ == "__main__":
    model = RetMIL(dim_in=1024, dim_hidden=512, num_heads=8, chunk_size=512, num_classes=2)
